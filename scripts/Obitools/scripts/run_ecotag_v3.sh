...
mkdir -p results/obi3/${marker}/
obi import --taxdump taxdump/taxdump.tar.gz results/obi3/${marker}/fish/taxonomy/my_tax
...
for fold in $(eval echo {1..$data_nfold}); do
  ...
  train_pref="results/obi3/${marker}/fish/train_f${fold}"
  test_pref="results/obi3/${marker}/fish/test_f${fold}"
  obi import $train_fa3 $train_pref
  obi import $test_fa $test_pref

  ## Build reference database
  train_db="${train_pref}_db"
  obi build_ref_db --taxonomy results/obi3/${marker}/fish/taxonomy/my_tax $train_pref $train_db
  ...
  obi ecotag --taxonomy results/obi3/${marker}/fish/taxonomy/my_tax --ref-database $train_db $test_pref $ecotag_res
  

  ## Convert ecotag result
  ecotag_tsv="results/obi3/${marker}/fold_${fold}_test_ecotag3.csv"
