...
def load_bert(bert_path, compile_=False):
    ...
    print("load_bert", bert_path)
    model = keras.models.load_model(bert_path, compile=False,
...
def seq2tokens(seq, token_dict, seq_length=250, max_length=None,
    ...
    tokens = [token_dict.get('[CLS]', 0)]  # début de séquence
    for word in seq:
        tokens.append(token_dict.get(word, token_dict.get('[UNK]', 1)))

    # Padding à droite jusqu’à max_length
    if len(tokens) < max_length:
        tokens += [token_dict.get('', 0)] * (max_length - len(tokens))
    else:
...
def predict(model, test_generator, roc_auc=False, classes=None,
...
def get_classes_and_weights_multi_tax(species_list, classes_preset=None, tax_ranks=['order', 'family'],
                                      unknown_thr=0, norm_weights=True):
