...
if __name__ == '__main__':
    ...
    classes, weight_classes, species_list_y = get_classes_and_weights_multi_tax(y_species,
                                                                                tax_ranks=['superkingdom', 'phylum','family'], unknown_thr=0)# args.num_test_samples+args.num_min_train_samples

    print(species_list_y[:, 2])
    ...
    if args.non_similar_selection:
        ...
        # pass
        df = pd.DataFrame(
            {"x": x, "y": y, "tax_id": y_species, "superkingdom": species_list_y[:, 0], "phylum": species_list_y[:, 1], "family": species_list_y[:, 2]})
        
        print(len(df))
        df_test = df.groupby("phylum").sample(frac=0.2, random_state=1)
