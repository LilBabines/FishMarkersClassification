...
for marker in ["ac16", "berry","mifish","teleo"] : #,
    ...
    for fold in range(1,7):
        ...
        # predictions = pd.DataFrame(columns=["order","family","obi4","MMseqs2","BERTax","DNAGPT","DNABert2_random_weights","DNABert2","BouillaBert","Ensemble_obi4","Ensemble_mmseqs2","top15_BouillaBert","sequence"])
        predictions = pd.DataFrame(columns=["order","family","MMseqs2","DNABert2","BouillaBert","sequence"])

        # DNABert2 : 
        dnabert2_preds = pd.read_csv(f"BouillaBert/experiments/fine_tune_taxa/outputs/from_dnabert2/config_{marker}/checkpoints/fold_{fold}/predictions.csv")
        test_df = pd.read_csv(f"BouillaBert/experiments/fine_tune_taxa/data/{marker}/folds/fold_{fold}/test.csv")
        ...
        # BouillaBert : 
        dnabert2_preds = pd.read_csv(f"BouillaBert/experiments/fine_tune_taxa/outputs/transductive/config_{marker}/checkpoints/fold_{fold}/predictions.csv")
        assert dnabert2_preds['labels_family_name'].equals(test_df['family'])
        predictions["BouillaBert"] = dnabert2_preds["preds_family_name"]

        # dnabert2_preds = pd.read_csv(f"BouillaBert/experiments/fine_tune_taxa/outputs/{marker}_from_random_weights/checkpoints/fold_{fold}/predictions.csv")
        # assert dnabert2_preds['labels_family_name'].equals(test_df['family'])
        ...
        # preds_bert = pd.read_csv(f'BouillaBert/experiments/fine_tune_taxa/outputs/transductive/config_{marker}/checkpoints/fold_{fold}/predictions.csv')
        # logits = pkl.load(open(f'BouillaBert/experiments/fine_tune_taxa/outputs/{transductive/config_{marker}/checkpoints/fold_{fold}/predictions.pkl', 'rb'))  
        # idx_to_label = (
        ...
        # assert predictions['BouillaBert'].equals(pd.Series(top15_families[:,-1])) # take the first family as prediction
       
        # predictions['top15_BouillaBert'] = top15_families.tolist()



        # predictions["Ensemble_obi4"] = predictions.apply(lambda row: row['obi4'] if row['obi4'] in row['top15_BouillaBert'] else row['BouillaBert'], axis=1)
        # predictions["Ensemble_mmseqs2"] = predictions.apply(lambda row: row['MMseqs2'] if row['MMseqs2'] in row['top15_BouillaBert'] else row['BouillaBert'], axis=1)
...
# for method in ["obi4","MMseqs2","BERTax","DNAGPT","DNABert2_random_weights","DNABert2","BouillaBert","Ensemble_obi4","Ensemble_mmseqs2"]: 
for method in ["MMseqs2","DNABert2","BouillaBert"]:
    print("==================")
